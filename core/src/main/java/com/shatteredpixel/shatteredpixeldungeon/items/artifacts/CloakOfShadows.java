...
public class CloakOfShadows extends Artifact {
	...
	public ArrayList<String> actions( Hero hero ) {
		...
		if ((isEquipped( hero ) || RogueTalent.activatesLostCloak(hero))
	...
	public void execute( Hero hero, String action ) {
		...
		if (action.equals( AC_STEALTH )) {
			...
			if (activeBuff == null){
				if (!isEquipped(hero) && !RogueTalent.activatesLostCloak(hero)) GLog.i( Messages.get(Artifact.class, "need_to_equip") );
	...
	public boolean doUnequip(Hero hero, boolean collect, boolean single) {
		if (super.doUnequip(hero, collect, single)){
			if (!collect || !RogueTalent.activatesLostCloak(hero)){
				if (activeBuff != null){
					activeBuff.detach();
					activeBuff = null;
				}
	...
	public boolean collect( Bag container ) {
		if (super.collect(container)){
			if (container.owner instanceof Hero
					&& passiveBuff == null
					&& RogueTalent.activatesLostCloak((Hero) container.owner)){
	...
	public void charge(Hero target, float amount) {
		if (charge < chargeCap) {
			if (!isEquipped(target)) amount *= RogueTalent.lightCloakUnequippedChargeFactor(target);
