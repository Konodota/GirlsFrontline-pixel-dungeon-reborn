...
public class v0_5_X_Changes {
    ...
    public static void add_0_5_9_2_Changes( ArrayList<ChangeInfo> changeInfos ) {
        ...
        changes = new ChangeInfo(Messages.get(ChangesScene.class, "changes"), false, null);
        changes.hardlight( CharSprite.POSITIVE );
        changes.fullWidth = true; //059v2 仅此一个分类，独占整行而不是只占左半区
        changeInfos.add(changes);
        ...
        //界面与机制调整：5页可翻页窗口（角色解锁/缩放/标题页/天赋重构/节日色彩）
        changes.addButton(new ChangeButton(new Object[]{Icons.get(Icons.DISPLAY), null, null, null, null},
                new String[]{"角色解锁方式调整", "PC端3.5档位缩放", "标题页交互优化", "天赋系统重构", "节日日期色彩"},
                new String[]{
        "_-_ _GSh-18_：使用_疫苗磁盘_解锁。\n"+
        ...
        "_-_ 局内菜单的_日期文本_现在会随节日显示_双色渐变_：圣诞节红绿、万圣节橙紫、面包节金棕、中秋节蓝白、复活节粉彩。\n"+
        "_-_ 自己设置的_蛋糕日_为金白渐变，普通日期为灰色微渐变。\n"
        }));
        ...
        "_-_ 更新了_隼_在存档列表（标题页存档槽与读档窗口）中的头像。\n"
        ));
        changes.addButton(new ChangeButton(new ScaleArmor(),
                "复合外骨骼重做",
        "_-_ _复合外骨骼_机制重做：不再占用第二个护甲栏，改为像_勋章_一样附着在护甲内部。\n"+
        "_-_ 在护甲上选择_复合_，可将一件_已鉴定且阶数低于当前护甲_的外骨骼复合进去，之后随时可以_查看_或_拆卸_。\n"+
        "_-_ 复合外骨骼的_刻印_照常生效；与基底为_同种刻印_时，刻印等级额外_+1_。\n"+
        "_-_ 复合_携带病毒_的外骨骼会使病毒转移到基底护甲，并破坏基底的正面刻印。\n"
        ));
        changes.addButton(new ChangeButton(new ItemSprite(ItemSpriteSheet.ARTIFACT_CAPE, null),
                "曼蒂装甲调整",
        "_-_ _曼蒂装甲_（荆棘斗篷）能量充满后_不再自动开启_偏斜力场，改为充满后可手动使用_开启力场_。\n"+
        "_-_ 现在即使不受伤，装甲也会随时间_缓慢自然充能_。\n"+
        "_-_ 新增_诅咒_效果_荆棘诅咒_：受伤时一半伤害转化为临时_最大生命值削减_，首次持续6回合，之后每次受伤延长3回合并叠加削减量。\n"
        ));
        changes.addButton(new ChangeButton(new ItemSprite(ItemSpriteSheet.AWP),
                "RF步枪标签",
        "_-_ 新增_RF_（步枪）武器标签：_SV-98_、_春田M1903_、_莫辛-纳甘_、_Denel NTW-20_、_Super SASS_、_WA2000_、_SRS-Covert_现带有RF标签，供角色天赋等系统按武器类别判定。\n"
        ));

        //其他调整为多页轮播按钮：最后一页为超级小爱DLC（蓝色解锁券贴图）
        Object[] miscIcons = new Object[]{
                new BlacksmithSprite(), null, null,
                new ItemSprite(ItemSpriteSheet.SuperAIDLC)
        };
        String[] miscTitles = new String[]{
                "其他调整", null, null, "超级小爱DLC"
        };
        String[] miscPages = new String[]{
                "_-_ 修复了_丹德莱_空手时可能报错的问题，空手默认为1F。\n"
              + "_-_ 修复了_双击GSh-18_导致崩溃的bug。\n"
              + "_-_ 修复了0层机密商店快捷出售商品时楼层错误导致的空指针。\n"
              + "_-_ 修复了机密商店购买二次确认后卡片未正常销毁、关闭窗口后仍遮挡屏幕区域导致无法点击移动的问题。\n"
              + "_-_ 击杀怪物时飘出的_经验值数字_现在会正确显示各类经验加成后的实际数值。\n",
              ...
              + "_-_ 修复了排行榜界面的若干显示问题。\n"
              + "_-_ 手机端新玩家的默认显示比例上限调整为_8X_（原为9X），仅影响从未手动调整过缩放时的自动默认值。\n"
              + "_-_ _旧版56-1式_在存档列表与角色选择界面的职业名特化为“_老练的561式_”，便于区分新旧模式。\n",

                "_-_ 隼新增转职_汉阳造88式-超级小爱_。暂未完成，目前暂时将资源预置出来。\n"
        };
        changes.addButton(new ChangeButton(miscIcons, miscTitles, miscPages,0.8F));
    }
    ...
    public static void add_0_5_9_Changes( ArrayList<ChangeInfo> changeInfos ) {
        ...
        "_-_ 卡牌名称按稀有度显示字体颜色：初始_白_、普通_绿_、稀有_蓝_、终阶_金_。\n"+
        "_-_ 初始武器为_M4A1_，可通过卡牌效果获得投掷技能和强化技能。\n"+
        "_-_ 喝下_神意合剂_时：不受对应阶的使用次数限制，每次使用都可为对应阶增加两个自由天赋点。选择1-3阶将额外获得该阶的两个随机天赋（该阶天赋已集齐则不可选），4阶始终可选。"
        ));
        ...
        "_-_ 获取规则：\n"+
        "_-_ 通关（取得护符）获得_2_电池，返程额外获得_3_电池。\n"+
        ...
        changes.addButton(new ChangeButton(new Hero(HeroClass.GSH18), 0.8F,
                "GSH18增强",
        "_-_ GSH18新增天赋_天狼星心脏_：激活后立刻抽离星之护盾转换为“_蓄能星击_”为下次攻击的附加伤害，附加伤害按天赋等级为护盾值的20%/40%/100%。\n"+
        "_-_ 天狼星心脏冷却时间改为阶梯式：+1为180回合，+2为120回合，+3为60回合。\n"+
        "_-_ _未来之星_便携手枪套件：装备HG标签武器时可放入便携手枪套件中，当目标在主武器射程外但在副武器（手枪）射程内时，会使用副武器进行快速攻击。\n"+
        "_-_ 新增_锁链冲击_与_蓄能星击_联动效果：_蓄能星击_造成的额外伤害可被_锁链冲击_传导给周围敌人。\n"+
        "_-_ _星之护盾_削弱：星盾的额外衰减阈值由角色等级*30，改为角色等级*10。\n"
        ));
        changes.addButton(new ChangeButton(new ItemSprite(ItemSpriteSheet.AN94),
                "武器调整",
        "_-_ _Gepard M6 Lynx_、_SRS-Covert_、_AN-94_的武器成长上调0.5。\n"+
        "_-_ _AN-94_装备_索敌_附魔时，射程额外+1。\n"+
        "_-_ _M99_的攻击速度调整为1回合。\n"+
        "_-_ 微调了_Cypros_的数值。\n"+
        "_-_ _.50口径_子弹不再会被小偷偷走，也不会被炸毁。\n"
        ));
        changes.addButton(new ChangeButton(Icons.get(Icons.DISPLAY),
                "主副武器系统",
        "_-_ 目前为未来之星专属被动\n"+
        "_-_ 新增_主副武器_与_换枪_机制：可同时携带主武器与副武器（_HG_标签手枪）并随时切换，切换状态可在存档中正确保存和恢复。\n"+
        "_-_ 现有手枪武器添加了_HG_标签（GSh-18、柯尔特M1911、纳甘左轮 M1895、维尔德mk.2、雷电、毛瑟C96、灰熊-MK-V、Cypros - 马格南婚礼），可作为副武器使用。\n"
        ));
        ...
        {
            miscChanges.add(
                    "_-_ 全新的_关于界面_：重制了关于页面的布局与内容。\n"
                    + "_-_ 调整了排名窗口中装备栏格子的显示大小。\n"
                    + "_-_ 同一状态在角色身上的特效不再重复叠加显示。\n"
                    + "_-_ 修复了部分选择窗口在按钮较多时可能超出窗口边界的问题。"
            );
            miscChanges.add(
                    "_-_ _天赋_贴图初步优化，统一了图标风格，并新增了符合少前风格的天赋边框UI。\n"
                    + "_-_ _天赋_文本进行了调整与修正：部分天赋描述中出现的角色名统一改为_角色_，避免与同名武器混淆。"
            );
            miscChanges.add(
                    "_-_ _德尔_使用了全新贴图，其_工单_消耗也进行了下调；0层的德尔移动到了_神秘据点_（404小屋）中，如果没有看到德尔，重建存档即可。\n"
                    + "_-_ _泛黄的袖珍本_的技能现在可以长按变成快捷行动，以便更频繁地使用。\n"
                    + "_-_ 消耗未祝福的_十字架_时，现在会打开遗失行囊所在位置的视野。"
            );
            miscChanges.add(
                    "_-_ 通过炼金产生的鉴定现在会展示被鉴定物品的名称。\n"
                    + "_-_ 使用放大镜查看物品时，若物品为堆叠形式且同格存在多个物品，将直接打开物品窗口查看。\n"
                    + "_-_ 未鉴定的武器/护甲也会正常显示依据其猜测等级的伤害与力量需求。\n"
                    + "_-_ _井房_的_水晶钥匙_现在显示在最上层，而不是被其他物品遮挡。"
            );
            miscChanges.add(
                    "_-_ 使用_蜕变协议_时，被替换天赋中已投入的1点天赋点将转换为自由天赋点返还（未加点则不返还）。\n"
                    + "_-_ 修复了_蜕变协议_的天赋选择/替换窗口在选项过多时无法滑动的问题，窗口现在可以正常滚动。"
            );
            miscChanges.add(
                    "_-_ _生日蛋糕_新增四种款式可选：薄荷巧克力、草莓奶油、烘焙可可、抹茶青苹果，可在节日蛋糕设置界面中选择。\n"
                    + "_-_ 重新实装了投掷武器_CZ75投掷斧_：1阶、力量需求9、无限耐久、回旋镖机制、可升级附魔，当前仅能从测试道具中获取（是的它回来了），投掷后会自动回到快捷栏原位置。\n"
                    + "_-_ _液金_现在可以通过快捷栏直接丢出，但需先手动丢出一次以更改默认行动。\n"
                    + "_-_ 指挥妖精、双生妖精、增员妖精新增独立物品图标。"
            );
            miscChanges.add(
                    "_-_ 提高了_红雷_的替换率，每区尾层概率刷出的下一区怪物改为在下一区首层的怪组中随机抽取。\n"
                    + "_-_ 修复了_兵蚁群_假死时也计入祭坛经验的问题，现在将在5回合后结算（首次假死不计入）。\n"
                    + "_-_ 主动拆除_NDB防御塔_（哨卫）时，现在可回收一定的充能。\n"
                    + "_-_ _G11_发射_诅咒子弹_触发嬗变时，现在会对G11内部的子弹配件进行嬗变。"
            );
            miscChanges.add(
                    "_-_ 修复了一系列_合剂_不触发使用药水天赋的问题。\n"
                    + "_-_ 修复了_图鉴_显示局内标签文本的问题。\n"
                    + "_-_ 修复了一处贴图为空可能导致的闪退问题。\n"
                    + "_-_ 修复了测试模式下未解锁天赋反复触发提示文本的问题。\n"
    ...
    public static void add_0_5_8_3_Changes( ArrayList<ChangeInfo> changeInfos ) {
        ...
        ArrayList<String> miscChanges = new ArrayList<>();
        {
            ...
            miscChanges.add(
                    "_-_ 草鞋同步破碎。\n"
                    + "_-_ 武力瞄准镜的辅助鉴定功能已完善。\n"
                    + "_-_ 加强+3超频校准，对低等级的瞄准镜保留+1的效果，冷却降低至3回合。\n"
                    +"_-_ 将镐子重新规范为普通三阶近战武器。\n"
            );
        }
        changes.addButton(new ChangeButton(new BlacksmithSprite(), 0.8F,
                 "其他调整", miscChanges));
    ...
    public static void add_0_5_7_3_To_0_5_8_1_Changes( ArrayList<ChangeInfo> changeInfos ) {
        ...
        changes.addButton(new ChangeButton(new Object[]{new ItemSprite(ItemSpriteSheet.ARTIFACT_HOLDER, null), new LloydsBeacon(), new HornOfPlenty(), new AlchemistsToolkit()},
                ...
                new String[]{"遗物在被破坏后将重新加入生成池子。\n\n钟表和刺刀虽然会因此额外生成道具，但是在拾取处已做处理，最高10级。",
                        ...
                        "炼金工具箱现在加入了一个功能，默认关闭。在打开后，在进入即将要生成上锁炼金房的新楼层时，有概率将其转化为隐藏炼金房或者普通隐藏房。\n\n上锁炼金房中的药水本质上是在生成时从本层的物品列表中取出一部分加入自身房间，移除上锁炼金房不会丢失本层的药水，这些药水依旧会在本层中生成，只会丢失模块(如果有)和炼金能量。"}));
    ...
    public static void add_0_5_5_3_Changes( ArrayList<ChangeInfo> changeInfos ){
        ...
        changes.addButton( new ChangeButton(new ItemSprite(ItemSpriteSheet.STONE_DISARM, null),
                "晰除模块",
		"_-_ 将_拆除模块_替换为了_晰除模块_，由_疫苗磁盘_拆解而来。\n" +
		"_-_ _晰除模块_保留丢出_拆除范围陷阱_的能力，并增加了可使用以_鉴定物品_是否带有_病毒_的能力。"
		));
    ...
    public static void add_0_5_5_2_Changes( ArrayList<ChangeInfo> changeInfos ){
        ...
        "_-_ 房间内的随机_模块/种子_奖励改为了_随机药水_奖励\n"+
        "_-_ 房间内的随机_药水/磁盘_奖励改为了_随机磁盘_奖励\n"+
        "_-_ 房间内的随机_武器/外骨骼_奖励改为了_稀有药水/磁盘/模块_奖励"
        ));
